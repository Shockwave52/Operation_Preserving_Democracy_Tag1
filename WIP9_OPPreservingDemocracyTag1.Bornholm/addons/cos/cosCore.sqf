...
for "_i" from 1 to _civilians do {
	...
	if (_i < _countPool) then { _tempUnit=_UnitList select _i; };						
	_tempPos=_roadPosArray select _rdCount;
	
	if ((_tempPos distance getMarkerPos (TDC_baseProt select 1)) > TDC_baseProt select 0) then 
	{		
		_rdCount=_rdCount+1;			
...
null =[_veh] execVM "addons\cos\addScript_Vehicle.sqf";
null =[_unit] execVM "addons\cos\addScript_Unit.sqf";
...
null =[_civilianArray,_PatrolVehArray,_roadPosArray] execVM "addons\cos\CosPatrol.sqf";

...
zbe_path = "addons\tdc_cache";

call compileFinal preprocessFileLineNumbers zbe_path + "\zbe_functions.sqf";
...
[] spawn  {
	while {true} do {
		...
		{
			_disable = _x getVariable zbe_path + "Disabled";
			_disable = if (isNil "_disable") then { false;
				} else {_disable;
				};
			if (!_disable && !(_x in zbe_cachedGroups)) then {
					zbe_cachedGroups = zbe_cachedGroups + [_x];
				 [zbe_aiCacheDist, _x, zbe_minFrameRate, zbe_debug] execFSM zbe_path + "\zbe_aiCaching.fsm";
				};
		} forEach allGroups;
	};
};
		...
		_assetscar = zbe_centerPOS nearEntities ["LandVehicle", zbe_mapside];
		{
			if !(_x in zbe_cached_cars) then {
				zbe_cached_cars = zbe_cached_cars + [_x];
					[_x, zbe_vehicleCacheDistCar] execFSM zbe_path + "\zbe_vehicleCaching.fsm";
			};
		} forEach _assetscar;
		_assetsair = zbe_centerPOS nearEntities ["Air", zbe_mapside];
		{
			if !(_x in zbe_cached_air) then {
				zbe_cached_air = zbe_cached_air + [_x];
				    [_x, zbe_vehicleCacheDistAir] execFSM zbe_path + "\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsair;
		_assetsboat = zbe_centerPOS nearEntities ["Ship", zbe_mapside];
		{
			if !(_x in zbe_cached_boat) then {
				zbe_cached_boat = zbe_cached_boat + [_x];
					[_x, zbe_vehicleCacheDistBoat] execFSM zbe_path + "\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsboat;
...
// if (!isDedicated) then {execFSM zbe_path + "\zbe_clientObjectDrawAuto.fsm";};
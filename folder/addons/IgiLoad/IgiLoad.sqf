...
if (isnil "IL_Variables") then
{
	...
	IL_Supported_Vehicles_HEMTT = ["B_Truck_Woodland_covered_F","B_Truck_Woodland_F","B_Truck_01_covered_F", "B_Truck_01_transport_F", "B_Truck_01_box_F", "Marinir_Truck_01_box_FG", "Marinir_Truck_01_transport_FG", "Marinir_Truck_01_covered_FG", "B_mas_mar_Truck_01_covered_F", "B_mas_mar_Truck_01_transport_F"];
	...
	IL_Supported_Vehicles_MOHAWK = ["I_Heli_Transport_02_F", "CH49_Mohawk_FG", "Marinir_CH49_Mohawk_FG","TDC_Heli_CH53D_Fleck","TDC_Heli_CH53D_Green"];
...
//	END IL_SetNewMass
	...
	IL_Vehicle_Chat =
	{
		...
		if (_mass_info && IL_Mass) then
		{
			...
			_headline = getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName");
			_text set [count _text, format["Fahrzeuggewicht: %1kg", round(_v_def_mass)]];
			_text set [count _text, format["Ladungsgewicht: %1kg", round(_cargo_mass)]];
			_text set [count _text, format["Gesamtgewicht: %1kg", round(_v_def_mass + _cargo_mass)]];				
		};
		if (vehicle Player != _v) then
		{
			if (_mass_info && IL_Mass) then
			{
				_text set [count _text, _msg];
				[player, "Logistik", [_text]] spawn TDC_fnc_messageTo;
			}
			else
			{
				[player, "Logistik", [_msg]] spawn TDC_fnc_messageTo;
			};
		}
		else
		{
...
//	END IL_Cargo_Para
	...
	IL_Do_Load =
	{
		...
		// CSE Carry check
		if (isClass(configFile >> "cfgPatches" >> "cse_sys_logistics")) then 
		{
			_objectCSE = [_cargo] call cse_fnc_getCarriedObj;
			if (!isNull _objectCSE) then 
			{
				[player, _objectCSE] call cse_fnc_carryObj;
			};			
		};
		...
		{
			{
				...
				{
					[_v, "Ladung zu schwer oder zu groß"] call IL_Vehicle_Chat;
				}
				else
				{
					if (IL_DevMod) then
					...
					{
						[_v, Format ["Beladen von ""%1"" auf ""%2""", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")]] call IL_Vehicle_Chat;
						_done = true;
						_counter = (_box_num);
						...
						{
							[_v, Format ["""%1"" auf ""%2"" geladen. Freie Plätze: ""%3"".", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName"), abs(_slot_num - _counter)], true] call IL_Vehicle_Chat;
						}
						else
						{
							[_v, Format ["""%1"" auf ""%2"" geladen. Kein weiterer freier Platz verfügbar.", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")], true] call IL_Vehicle_Chat;
						};
